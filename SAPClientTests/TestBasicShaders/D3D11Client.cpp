...
#include "Globals.h"
#include "Scene.h"
#include "D3D11Utils.h"  // for make_unique<>()
...
D3D11Client::clbkCreateRenderWindow()
{
    ...
    if (!data->fullscreen) {
		...
		// Currently this code path does not work - Orbiter switches to fullscreen.
		// This is because of oapi::GraphicsClient::clbkCreateRenderWindow() which
		// produces a window which is fullscreen and cant be moved around like a 
		// normal window. So for now do not set full screen in the Launchpad Video tab.

        // First a style setting, window should stay on top
        auto currExStyle = GetWindowLong(m_hWindow, GWL_EXSTYLE);
        currExStyle |= WS_EX_TOPMOST;
        //SetWindowLong(m_hWindow, GWL_EXSTYLE, currExStyle);
		SetWindowLongPtr(m_hWindow, GWL_STYLE, WS_BORDER | WS_CAPTION);
    ...
    // Initialize shader version strings
    m_Config.initShaderLevels();

    // Create the Scene & initialize it
    m_pScene = ClientUtils::make_unique<Rendering::Scene>(*this);
    m_pScene->init3D();

    // Create ShaderManager now that scene rendering is about to begin
	m_pShaderManager = ClientUtils::make_unique<Rendering::ShaderManager>(m_Config.getD3DDevice());
...
D3D11Client::clbkRenderScene()
{
    ...
    // Scene rendering
    m_pScene->renderStarsAndPlanets();
...
void D3D11Client::clbkPreOpenPopup()
{
    ...
    auto data = GetVideoData();
    if (data->fullscreen) {
		// Uses the D3D11 swapchain to set the fullscreen state - NOT SUPPORTED
		//m_pScene->setFullScreenState();
    }
}
...
bool D3D11Client::clbkFullscreenMode() const
{
    ...
    return false; //m_Config.bFullScreenWindow;
}
