...
public:
    ...
    Rendering::Scene * getScene(void) { return m_pScene.get(); }

    void progressString( const char *line, DWORD _color );

    /* Shader loading utilities
     * This is just for redirecting the call to ShaderManager
     * Its templated for various shader types
     */
    template <typename T>
    ...
    {
		return m_pShaderManager->getShader<T>(fileName, entryPoint, result, pDefines);
    }
    ...
    {
        return m_pShaderManager->getShaderBytecode(shaderObject, bytecode);
    }
...
private:
    ...
    /*
     * There is the only scene in the entire client. We allocate this dynamically
     * because this needs to be allocated only after client requests rendering.
     * We could have used std::unique_ptr here.
     */
    std::unique_ptr<Rendering::Scene> m_pScene;

    /*
     * This need not exist till its allocated in clbkCreateRenderWindow()
     * Client exists as soon as its loaded in LaunchPad - no need to delay that
     * with unnecessary memory allocation of ShaderManager. So we dynamically
     * allocate.There is only 1 ShaderManager in the entire client.
     */
    std::unique_ptr<Rendering::ShaderManager> m_pShaderManager;
};
