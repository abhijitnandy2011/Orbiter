...
void VideoTab::selectOutput() 
{
	...
	for( DWORD j = 0; j < NumModes; j++ ) {
		...
		if( m_DXGIModeDesc[j].ScanlineOrdering != 1 )
			continue;
		sprintf_s( line, "%u x %u %u Hz %u %u",
			m_DXGIModeDesc[j].Width,
			m_DXGIModeDesc[j].Height, 
			(int)((float)m_DXGIModeDesc[j].RefreshRate.Numerator/(float)m_DXGIModeDesc[j].RefreshRate.Denominator + 0.3 ),
			m_DXGIModeDesc[j].Scaling, 
			m_DXGIModeDesc[j].ScanlineOrdering );
